...
2. `per_node_value`, `table_slice`, `table_update_oracle`, `dependency_evidence`, `invalidation_evidence`, and `retained_artifact_ref` currently use exact JSON comparison with family-specific policy ids and mismatch labels.
...
12. Full third-pass W056 closure remains limited by typed non-comparable surfaces: DnaTreeCalc retained empty-body/transition artifacts and lifecycle artifacts are now present, and DnaTreeCalc dynamic/cross-workspace artifacts are retained, but full namespace/anchor/workspace cross-producer pairing, `execution_outcome.class_id` on older OxXlPlay table artifacts, and direct Excel dependency/invalidation internals remain open or typed unavailable.
