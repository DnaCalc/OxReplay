...
4. `Diff`
   ...
   - host-rollout comparison planning extends this family to typed `per_node_value`, `table_slice`, `dependency_evidence`, and `invalidation_evidence` view families once retained artifacts publish them.
5. `Explain`
   ...
   - explanation over dependency and invalidation evidence must preserve source-lane provenance and must not infer TreeCalc-private or OneCalc-private semantics.
6. `Distill`
