...
12. Full third-pass W056 closure remains blocked by `BLK-REPLAY-004`: OxXlPlay now retains explicit empty-body table, first-row insert, last-row delete, empty-column rename, current-row absence, and collision observations, and DnaTreeCalc activates empty-body cases through LiveOxCalc, but retained DnaTreeCalc empty-body/transition artifacts are still needed for paired comparison. Lifecycle callback artifacts, complete namespace/anchor/workspace cross-producer pairing, `execution_outcome.class_id` on older OxXlPlay table artifacts, and direct Excel dependency/invalidation internals also remain open or typed unavailable.
