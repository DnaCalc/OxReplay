# HANDOFF_OXXLPLAY_001_OBSERVATION_SEAM

## Header
1. handoff id: `HANDOFF_OXXLPLAY_001`
2. date: `2026-03-20`
3. from repo: `OxReplay`
4. to repo: `OxXlPlay`
5. related workset or feature id: `OxXlPlay initial integration seam`

## Purpose
State the initial `OxReplay` to `OxXlPlay` contract for Excel observation artifacts entering the Replay appliance.

## Proposed change
`OxXlPlay` should target replay-ready observation artifact emission from the start.
...
## Current evidence
1. seam packet: `docs/spec/OXREPLAY_OXXLPLAY_OBSERVATION_SEAM.md`
2. outbound note: `docs/upstream/NOTES_FOR_OXXLPLAY.md`
3. current `DNA ReCalc` and retained baseline surfaces under `docs/test-corpus/` and `docs/test-runs/`

## Impact
1. capability impact: enables future `OxXlPlay` `C0` and later capability validation without widening `OxReplay` semantics ownership
...
4. affected repos or hosts: `OxXlPlay`, `OxReplay`, `DNA ReCalc`, later `OxCalc` differential runs

## Requested response
1. acknowledge whether `OxXlPlay` will target direct canonical bundle emission first or a narrower observation-contract phase first
...
1. `OxXlPlay` responded through `../OxXlPlay/docs/upstream/NOTES_FOR_OXREPLAY.md`
